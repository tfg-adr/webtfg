  ...
  <div class="center-panel">
    ...
    <div v-if="activeTab === 'clientes'" class="tab-content">
      ...
      <div class="clientes-search-wrap">
        <input class="clientes-search" type="text" placeholder="Filtrar por nombre o DNI..."
          v-model="busquedaClientes" @input="onBusquedaClientes" />
      </div>
      ...
      <template v-else>
        <div class="clientes-list" v-if="clientes.length > 0">
          <div class="cliente-row" v-for="c in clientes" :key="c.id_cliente" @click="abrirPerfil(c.id_cliente)">
            <div class="cliente-avatar" :style="{ background: avatarColor(c.nombre) }">{{ initials(c.nombre) }}</div>
        ...
        <div class="pagination" v-if="totalPaginas > 1">
          ...
          <button class="page-btn" v-for="p in paginasVisibles" :key="p"
            :class="{ active: p === paginaClientes }" @click="cambiarPagina(p)">{{ p }}</button>
...
async function abrirPerfilPorSuscripcion(q) {
  try {
    ...
    const { data } = await api.get('/cliente/buscar', {
      params: { q: q.nombre, id_gym }
    })
...
watch(activeTab, (val) => {
  if (val === 'clientes' && clientes.value.length === 0) fetchClientes()
})
...
function onSearch() {
  ...
  searchTimer = setTimeout(async () => {
    try {
      const { id_gym } = getUsuario()
      const { data } = await api.get('/cliente/buscar', {
        params: { q: searchQ.value, id_gym }   // ← filtra por compañía via id_gym
      })
...
async function fetchResumen() {
  ...
  try {
    ...
    const { data } = await api.get('/suscripcion/resumen', { params: { idCompania: id_compania } })
    resumen.value = data
...
async function fetchBars() {
  ...
  try {
    const { id_compania } = getUsuario()
    const { data } = await api.get('/suscripcion/ingresos-mensuales', { params: { idCompania: id_compania } })
    barData.value = data
  } catch (e) { console.error(e) }
...
async function fetchCuotas() {
  ...
  try {
    const { id_compania } = getUsuario()
    const { data } = await api.get('/suscripcion/recientes', { params: { idCompania: id_compania } })
    cuotas.value = data
  } catch (e) { console.error(e) }
...
async function fetchGymId() {
  const { id_gym } = getUsuario()
  gymId.value = id_gym ?? null
}

const paginasVisibles = computed(() => {
  const pages = []
  const desde = Math.max(1, paginaClientes.value - 2)
  const hasta  = Math.min(totalPaginas.value, paginaClientes.value + 2)
...
function setFiltro(f) { filtroClientes.value = f; paginaClientes.value = 1; fetchClientes() }

function onBusquedaClientes() {
  clearTimeout(busquedaTimer)
  busquedaTimer = setTimeout(() => { paginaClientes.value = 1; fetchClientes() }, 300)
}
...
/* ── TAB CLIENTES ─────────────────────────────────────── */
.clientes-toolbar { display: flex; justify-content: space-between; align-items: center; margin-bottom: 14px; flex-wrap: wrap; gap: 10px; }
.filtro-tabs { display: flex; gap: 6px; }
.filtro-btn { background: #16161c; border: 1px solid #1e1e2a; border-radius: 8px; padding: 6px 14px; font-family: 'DM Sans', sans-serif; font-size: 12px; font-weight: 500; color: #55556a; cursor: pointer; transition: all 0.2s; }
.filtro-btn:hover  { color: #d8d8e8; border-color: #2e2e3e; }
.filtro-btn.active { background: var(--color-primary-bg); border-color: var(--color-primary-border); color: var(--color-primary); }
.clientes-meta { font-size: 12px; color: #33334a; }
.clientes-search-wrap { margin-bottom: 14px; }
.clientes-search { width: 100%; background: #16161c; border: 1px solid #1e1e2a; border-radius: 10px; padding: 9px 14px; color: #d8d8e8; font-family: 'DM Sans', sans-serif; font-size: 13px; outline: none; transition: border-color 0.2s; box-sizing: border-box; }
.clientes-search::placeholder { color: #2e2e3a; }
.clientes-search:focus { border-color: var(--color-primary); }
.clientes-list { display: flex; flex-direction: column; gap: 6px; margin-bottom: 16px; }
.cliente-row { display: flex; align-items: center; gap: 12px; padding: 10px 14px; background: #16161c; border: 1px solid #1e1e2a; border-radius: 10px; cursor: pointer; transition: all 0.18s; }
.cliente-row:hover { border-color: var(--color-primary-border); background: var(--color-primary-bg-sm); transform: translateX(2px); }
.cliente-avatar { width: 36px; height: 36px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-family: 'Bebas Neue', sans-serif; font-size: 13px; color: #fff; flex-shrink: 0; }
.cliente-info { display: flex; flex-direction: column; gap: 2px; flex: 1; min-width: 0; }
.cliente-nombre { font-size: 13px; font-weight: 600; color: #d8d8e8; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.cliente-sub { font-size: 11px; color: #44445a; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.cliente-plan-col { display: flex; flex-direction: column; align-items: flex-end; gap: 2px; min-width: 100px; flex-shrink: 0; }
...
.cliente-badge { font-size: 10px; font-weight: 700; padding: 3px 9px; border-radius: 20px; flex-shrink: 0; white-space: nowrap; }
...
.pagination { display: flex; justify-content: center; align-items: center; gap: 6px; padding-top: 8px; }
.page-btn { background: #16161c; border: 1px solid #1e1e2a; border-radius: 7px; padding: 5px 11px; color: #55556a; font-family: 'DM Sans', sans-serif; font-size: 12px; cursor: pointer; transition: all 0.2s; min-width: 32px; }
.page-btn:hover:not(:disabled) { border-color: var(--color-primary); color: var(--color-primary); }
.page-btn.active { background: var(--color-primary-bg); border-color: var(--color-primary-border); color: var(--color-primary); font-weight: 700; }
